...
async function check() {
  const { error } = await supabase.from('profiles').update({ public_email: 'test@test.com' }).eq('id', '123');
  console.log('public_email:', error);
  const { error: e2 } = await supabase.from('profiles').update({ contact_email: 'test@test.com' }).eq('id', '123');
  console.log('contact_email:', e2);
}
